...
云铝溢鑫设备维修管理平台，后端与 Dify 智能体 API 互动。
...
- **人员管理**：登录认证、角色区分（报修人/检修人/管理员）、密码重置
...
3. 双击运行 `start.bat`，脚本会自动完成：
   ...
   - 安装前后端依赖（首次运行）
   - 初始化数据库（首次运行）
   - 启动后端和前端（单终端管理，不弹额外窗口）

4. 浏览器打开控制台提示的地址，如 `http://192.168.1.100:3000`

5. **停止服务**：在启动终端中按任意键，脚本会自动关闭所有服务
...
├── start.bat              # Windows 一键启动（开发模式，单终端管理）
├── start-prod.bat         # Windows 一键启动（生产模式，单终端管理）
├── client/                # Vue 3 前端
│   ├── src/
│   │   ├── api/           # API 请求层
│   │   ├── assets/styles/ # 蓝白企业主题
│   │   ├── components/    # 通用/布局组件
│   │   ├── views/         # 页面
│   │   ├── stores/        # Pinia 状态
│   │   ├── types/         # TypeScript 类型
